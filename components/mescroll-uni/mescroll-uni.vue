<template>
	<view class="mescroll-uni-warp" :style="'height:'+m_height">
		<scroll-view :id="viewId" class="mescroll-uni" :class="{'mescroll-uni-fixed':fixed}" :style="{'padding-top':padTop,'padding-bottom':padBottom,'top':fixedTop,'bottom':fixedBottom,'height':m_height}" :scroll-top="scrollTop" :scroll-with-animation="scrollAnim" @scroll="scroll" @touchstart="touchstartEvent" @touchmove="touchmoveEvent" @touchend="touchendEvent" @touchcancel="touchendEvent" :scroll-y='scrollAble' :throttle="mescroll.optUp.onScroll==null" :enable-back-to-top="true">
			<view class="scroll-view_content" :style="{'transform': translateY, 'transition': transition,'height':m_height}">
				<!-- 下拉加载区域-->
				<view v-if="mescroll.optDown.use" class="mescroll-downwarp">
					<view class="downwarp-content">
						...
						<text class="downwarp-tip">{{downText}}</text>
					</view>
				</view>
				...
				<view v-if="isShowEmpty" class="mescroll-empty" :class="{'empty-fixed':optEmpty.fixed}" :style="{'z-index':optEmpty.zIndex,'top':optEmpty.top}">
					...
					<text v-if="optEmpty.tip" class="empty-tip">{{optEmpty.tip}}</text>
					<text v-if="optEmpty.btnText" class="empty-btn" @click="emptyClick">{{optEmpty.btnText}}</text>
				</view>
				...
				<view v-if="mescroll.optUp.use" class="mescroll-upwarp">
					...
					<template v-if="isUpLoading">
						...
						<text class="upwarp-tip">{{mescroll.optUp.textLoading}}</text>
					</template>
					<!-- 无数据 -->
					<text v-if="!isDownLoading && isUpNoMore" class="upwarp-nodata">{{mescroll.optUp.textNoMore}}</text>
				</view>
			</view>
		</scroll-view>
