		...
		<view :class="{'uni-swipe-action--show':isShowBtn}" :style="{'transform':transformX,'-webkit-transform':transformX}"
		 ...
		 @touchcancel="touchEnd" @click="bindClickCont">
			...
			<view :id="elId" class="uni-swipe-action__btn-group">
				<div v-for="(item,index) in options" :key="index" :style="{backgroundColor: item.style && item.style.backgroundColor ? item.style.backgroundColor : '#C7C6CD',color: item.style && item.style.color ? item.style.color : '#FFFFFF',fontSize: item.style && item.style.fontSize ? item.style.fontSize : '28upx'}"
				 class="uni-swipe-action--btn" @click.stop="bindClickBtn(item,index)">
					{{ item.text }}
				</div>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			endMove(is_opne = this.isShowBtn) {
				if (this.direction === 'Y' || this.disabled) {
					this.direction = ''
					return
				}
