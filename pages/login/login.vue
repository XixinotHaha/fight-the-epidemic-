<template>
	<!-- 登录页面 -->
	<view>
		<view class="logo"><image mode="aspectFill" src="/static/center/LGlogo.png"></image></view>
		<!-- 账号密码输入框 -->
		<view class="form">
			<view class="username">
				<image src="/static/login/mobile.png" mode="" class="image"></image>
				<input @focus="fixedToggle" @blur="fixedToggle(true)" maxlength="11" :placeholder="data.mobile" type="number" v-model="mobile" placeholder-style="color:#999999" />
			</view>
			<view class="password">
				<view class="image"><image src="/static/login/psd.png" mode=""></image></view>
				<input
					@focus="fixedToggle"
					@blur="fixedToggle(true)"
					maxlength="12"
					:placeholder="data.psd"
					v-model="password"
					:password="is_password"
					placeholder-style="color: #999999"
				/>
				<view class="watch" @tap="toShow">
					<image src="/static/login/hide.png" mode="" v-if="is_password" style="width: 30upx; height: 22upx;"></image>
					<image src="/static/login/show.png" mode="" v-else style="width: 30upx; height: 22upx;"></image>
				</view>
			</view>
			<!-- <view class="res">
				<view @tap="toPage('mobile_login')" class="active-con">验证码登录</view>
			</view> -->
			<view class="btn" @tap="doLogin">{{ data.loginBtn }}</view>
			<view class="result">
				<view @tap="toPage('register')" class="active-con1">{{ data.toRegister }}</view>
				<!-- <view class="line">|</view>
				<view @tap="toPage('resetpassword')" class="active-con2">{{ data.forgetPsd }}</view> -->
			</view>
		</view>
	</view>
</template>

<script>
var _self;
let app = getApp();
export default {
	data() {
		return {
			data: getApp().globalData.data.login,
			mobile: '',
			password: '',
			is_password: true,
			isShowOauth: true,
			showProvider: {
				weixin: true,
				qq: true,
				sinaweibo: false,
				xiaomi: false
			},
			windowHeight: '', //设备高度
			hasFixed: true, // 是否显示底部定位元素
			province: '',
			industry: ''
		};
	},
	onShow() {},
	onLoad(options) {
		_self = this;
		_self.mobile = _self.$Route.query.mobile;
		let industry = uni.getStorageSync('industry');
		if(industry.name=="chinese"){
			this.data = this.$chinese[0].login
		}else{
			this.data = this.$chinese[1].login
		}
	},
	methods: {
		toShow() {
			this.is_password = !this.is_password;
		},
		toPage(page) {
			uni.hideKeyboard();
			if (page == 'mobile_login') {
				this.$Router.replace({
					name: page,
					params: {}
				});
				return;
			}
			this.$Router.push({
				name: page,
				params: {}
			});
		},
		// 选择语言
		updateFiledValues(){
			let industry = uni.getStorageSync('industry');
			_self.$ajax({
				path: _self.$api.update_filed_values,
				data: {
					default_language: industry.name=="chinese"?1:2, //语言
				},
				success(res) {
					let { data, code, msg } = res.data;
					console.log(data);
				}
			});
		},
		doLogin() {
			uni.hideKeyboard();
			// //验证手机号码
			if (!/^1(3|4|5|6|7|8|9)\d{9}$/.test(this.mobile)) {
				uni.showToast(
					!this.mobile
						? {
								title: this.data.mobile,
								icon: 'none'
						  }
						: {
								title: this.data.rightMobile,
								icon: 'none'
						  }
				);
				return false;
			}
			if (this.password.length < 6 || this.password.length > 12) {
				uni.showToast({
					title: this.data.psd,
					icon: 'none'
				});
				return false;
			}
			uni.showLoading({
				title: this.data.inSubmit
			});
			const userdata = {
				account: this.mobile,
				password: this.password,
				//#ifdef APP-PLUS
				uuid: plus.device.uuid
				//#endif
			};
			// console.log(_self.$api.login, userdata);
			_self.$ajax({
				path: _self.$api.login,
				data: userdata,
				success(res) {
					let { data, code, msg } = res.data;
					uni.hideLoading();
					if (code == 1) {
						console.log(data.userinfo);
						uni.setStorageSync('USERINFO', data.userinfo);
						_self.$req.defaultReq.baseData['token'] = data.userinfo.token;
						_self.updateFiledValues()
						if(data.userinfo.is_info==0){
							_self.$Router.push({
								name: 'user_info_input',
								params: {}
							});
						}
						if(data.userinfo.is_info==1){
							_self.$Router.push({
								name: 'check_basic',
								params: {}
							});
						}
						// //先判断是不是第一次登陆,如果是第一次登陆就去完善个人信息
						// if(data.userinfo.is_info==0 && data.userinfo.is_zz==0){
						// 	_self.$Router.push({
						// 		name: 'user_info_input',
						// 		params: {}
						// 	});
						// }
						// // 如果不是第一次登陆就去首页
						// if(data.userinfo.is_info==1 && data.userinfo.is_zz==1){
						// 	_self.$Router.replaceAll({
						// 		name: 'index',
						// 		params: {}
						// 	});
						// }
						// // 如果填了信息,没等级症状,就去等级症状页
						// if(data.userinfo.is_info==1 && data.userinfo.is_zz==0){
						// 	_self.$Router.push({
						// 		name: 'check_basic',
						// 		params: {}
						// 	});
						// }
						
					} else {
						uni.showToast({
							title: msg,
							icon: 'none'
						});
					}
				}
			});
		},
		fixedToggle(has) {
			//是否显示三方登录
			this.tabbar = has || false;
		}
	}
};
</script>
<style lang="scss">
@font-face {
	font-family: 'HMfont-login';
	src: url('data:application/x-font-woff2;charset=utf-8;base64,')
		format('woff2');
}

.active-con {
	color: #2abac2 !important;
	font-size: 28upx;
	margin-top: 28upx;
}

.active-con1 {
	color: #2abac2 !important;
	font-size: 28upx;
}

.active-con2 {
	color: #333333 !important;
	font-size: 28upx;
}

.icon {
	font-family: 'HMfont-login' !important;

	font-style: normal;
	color: #ffffff;

	&.weixin {
		&:before {
			content: '\e615';
		}
	}

	&.qq {
		&:before {
			content: '\e752';
		}
	}

	&.weibo {
		&:before {
			content: '\e63d';
		}
	}

	&.xiaomi {
		&:before {
			content: '\e661';
		}
	}
}

// page {
// 	height: 100%;
// }

.logo {
	width: 100%;
	padding: 180upx 0 100upx;
	display: flex;
	justify-content: center;
	align-items: center;

	image {
		width: 172upx;
		height: 172upx;
		border-radius: 40upx;
		box-shadow: 0px 0px 20px rgba(136, 136, 136, 0.2);
	}
}
.toselect {
	// position: absolute;
	// top: 44upx;
	// righr:32upx;
	text-align: right;
}

.form {
	padding: 0 50upx !important;
	.res {
		display: flex;
		justify-content: flex-end;
		align-items: center;
		color: #ffffff !important;
	}

	.result {
		display: flex;
		justify-content: center;
		align-items: center;
		margin-top: 39upx;

		.line {
			color: #333;
			font-size: 24upx;
			margin: 0 50upx;
		}
	}

	width: 100%;
	padding: 0 75upx;
	font-size: 30upx;

	.username,
	.password,
	.code {
		width: calc(100%);
		height: 116upx;
		display: flex;
		align-items: center;
		border-bottom: 2upx solid rgba(238, 238, 238, 1);

		image {
			width: 28upx;
			height: 38upx;
			margin-right: 20upx;
		}

		input {
			width: 100%;
			font-size: 28upx;
			font-weight: 400;
			color: rgba(153, 153, 153, 1);
		}
	}

	.btn {
		color: #fff;
		width: 100%;
		height: 90upx;
		display: flex;
		justify-content: center;
		align-items: center;
		background-color: #2abac2;
		font-size: 40upx;
		border-radius: 45upx;
		border: 1upx solid #2abac2;
		margin-top: 80upx;
	}
}

.oauth {
	position: absolute;
	bottom: 100upx;
	width: 100%;

	.text {
		width: 100%;
		height: 60upx;
		display: flex;
		justify-content: center;
		align-items: center;
		color: rgba(153, 153, 153, 1);
		font-size: 28upx;
	}

	.list {
		width: 100%;
		display: flex;
		justify-content: center;
		align-items: center;

		.weixin {
			font-size: 80upx;
			margin: 0 30upx;
			width: 72upx;
			height: 72upx;

			image {
				width: 72upx;
				height: 72upx;
			}
		}
	}
}
</style>
