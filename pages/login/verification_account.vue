<template>
	...
	<view>
		<uni-title class="nav">
			...
			<view slot="center" class="tab-bar" style="font-size: 18px;">{{ data.verification_account }}</view>
		</uni-title>
		<view class="form">
			<view class="username">
				<input type="number" :placeholder="currentMobile" disabled  maxlength="11" placeholder-style="color:#999999" />
			</view>
			<view class="code">
				<view class="input">
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			this.currentMobile = this.data.currentPhone + this.mobile
		...
		methods: {
			...
			async doReg() {
				...
				_self.$ajax({
					path: this.$api.verification_account,
					data: {
						mobile:_self.mobile,
						event:"login",
						captcha:_self.code
